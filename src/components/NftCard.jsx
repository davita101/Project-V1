            ...
            {productText.map((item, index) => {
                if (item.nftJson) {
                    return item.nftJson.map((category, index) => (
                        <div onMouseOver={() => { setIndexCard(index), setOver(true) }} onMouseLeave={() => { setIndexCard(index), setOver(false) }} className='grid gap-[.6rem] p-[1rem] bg-white shadow-lg overflow-hidden rounded-2xl cursor-pointer transition-all hover:translate-y-[-1.5rem] hover:shadow-xl duration-[.1s] hover:bg-blue-400 ease-in '>
